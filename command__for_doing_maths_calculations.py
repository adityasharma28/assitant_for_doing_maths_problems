...
def subt(query):
    ...
    return q
def mul(x):
    t="times" if "times" in x else "multiply"
    l=query.split(t)
    c=[]
    for k in l:
        n=''
...
def div(r):
    t="divide by" if "divide by" in r else "divide"
    l=query.split(t)
    c=[]
    for k in l:
        n=''
...
if __name__ == "__main__":
    ...
    while True:
        if 1:
            ...
            if "times" or "multiply" in query:
                y=mul(query)
                speak(y)
            elif "add" or "sum" in query:
                a=add1(query)
                speak(a)
            elif "from" in query:
                v=subt(query)
                speak(v)
            elif "divide by" or "divide" i query:
                g=div(query)
                k=speak(g)
